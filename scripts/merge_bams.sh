...
cd ~/tonsa_genomics/data/aligned/lane2

for i in $(ls *.bam | cut -f 1-3 -d "_" | uniq )

do {

  samtools merge ~/tonsa_genomics/data/aligned/merged/${i}.bam \
      ~/tonsa_genomics/data/aligned/${i}.bam \
      ~/tonsa_genomics/data/aligned/lane2/${i}_lane2.bam
...
echo ${i},$first,$ONE,$TWO,$THREE,$FOUR,$AVG,$STD,$COV
 ...
 } >> ~/tonsa_genomics/analysis/count.aligned_merged.txt
